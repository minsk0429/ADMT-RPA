...
| A1TCP_88_v2 | **0.8252** | **23.66** | Iteration 10,230 (Dice: 0.702) |

- **분석**: v2가 v1보다 0.36% 높은 Dice Score 달성
- v2가 v1보다 더 일찍 CutMix로 전환(약 1,848회 더 빠름)
- 88개 라벨 데이터에서는 조기 전환이 더 효과적
...
| A1TCP_176_v2 | 0.8375 | 24.11 | Iteration 11,310 (Dice: 0.751) |

- **분석**: v1이 v2보다 1.01% 높은 Dice Score 달성
- v1이 v2보다 훨씬 더 일찍 CutMix로 전환(약 5,452회 더 빠름)
- 176개 라벨 데이터에서는 매우 조기의 CutMix 전환이 최고 성능 달성

### 주요 발견사항

1. **라벨 데이터 양에 따른 전략 차이**
   - 적은 라벨 데이터(44개): 중간~늦은 전환 (Iter 12,765 ~ 13,455)
   - 중간 라벨 데이터(88개): 조기 전환이 더 효과적 (Iter 10,230)
   - 많은 라벨 데이터(176개): 매우 조기 전환이 최고 성능 (Iter 5,858)
...
3. **증강 전환 타이밍의 중요성**
   - 라벨 데이터가 많을수록 더 일찍 CutMix로 전환하는 것이 유리
   - 44개: Iter 12,765 ~ 13,455 / 88개: Iter 10,230 ~ 12,078 / 176개: Iter 5,858 ~1 1,310
